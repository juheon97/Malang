...
function CounselChannelRoom() {
  ...
  const increaseMaxUsers = () => {
    setFormData(prev => ({...prev, maxUsers: Math.min(parseInt(prev.maxUsers) + 1, 4).toString()}));
  };

  const decreaseMaxUsers = () => {
    setFormData(prev => ({...prev, maxUsers: Math.max(parseInt(prev.maxUsers) - 1, 1).toString()}));
  };
  ...
  const handleSubmit = (e) => {
    ...
    // API 호출 로직 추가
    navigate('/counsel-channel-video'); // 경로 변경
  };
  ...
  <span className="text-sm font-semibold text-gray-700 ">상담채널</span>
          ...
          {/* 음성채널 생성 폼  <h2 className="text-xl font-medium text-gray-800 mb-6">음성채널 생성</h2> */}
