  ...
  >
    ...
    {participant.name && renderParticipantInfo ? (
      ...
      <div className="absolute bottom-2 right-2 bg-gray-700 bg-opacity-70 text-white px-2 py-1 rounded text-xs">
        ...
        {participant.isSelf && <span className="ml-1 bg-blue-500 px-1 py-0.5 rounded text-xs">Me</span>}
      </div>
    )}
  </div>
);
...
// 비디오 레이아웃 컴포넌트
const VoiceVideoLayout = ({ participants = [], renderParticipantInfo }) => {
  ...
  // 디버깅을 위한 로그
  useEffect(() => {
    console.log('Participants 구조:', validParticipants.map(p => ({
      id: p.id,
      stream: !!p.stream,
      isSelf: p.isSelf,
      name: p.name
