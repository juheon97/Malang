...
const ReviewModal = ({ counselor, onClose }) => {
  ...
  const [reviews, setReviews] = useState([
    ...
    },
    {
      ...
      userId: "익명의 리뷰어",
      date: "2025.03.12",
      content: "선생님의 조언이 정말 도움이 되었습니다. 다음에도 상담 받고 싶어요.",
      likes: 8,
      isLiked: false
    },
    {
    ...
    },
    {
      ...
      userId: "익명의 리뷰어",
      date: "2025.03.05",
      content: "정말 전문적으로 상담해주셨어요. 현실적인 조언들이 큰 도움이 됐습니다.",
      likes: 15,
      isLiked: false
    }
  ]);
        ...
        <div className="relative pt-12 pb-4 rounded-lg overflow-hidden mx-10">
          ...
          >
            <div className="flex items-center justify-center">
              {/* 상담사 프로필 이미지 */}
              <div className="relative w-16 h-16 rounded-full bg-[#FCE2D0] border-2 border-white overflow-hidden mr-8">
          ...
          <div className="mt-6 mb-2 flex justify-end">
            <div className="flex items-center space-x-4 text-xs">
              ...
              >
                <span className={`mr-1 ${sortType === 'latest' ? 'text-gray-800 font-bold' : 'text-gray-400'}`}>•</span>
                <span className={sortType === 'latest' ? 'text-gray-800' : 'text-gray-400'}>최신순</span>
              </div>
              <div 
                className="flex items-center cursor-pointer"
                onClick={() => setSortType('helpful')}
              >
                <span className={`mr-1 ${sortType === 'helpful' ? 'text-gray-800 font-bold' : 'text-gray-400'}`}>•</span>
                <span className={sortType === 'helpful' ? 'text-gray-800' : 'text-gray-400'}>도움순</span>
              </div>
            </div>
          </div>
          
          {/* 리뷰 목록 */}
          <div className="overflow-y-auto scrollbar-hide h-[calc(90vh-230px)] py-2" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {sortedReviews.map((review) => (
              <div 
                key={review.id} 
                className="bg-[#F9FFF8] p-4 mb-4 rounded"
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-green-600 font-medium">{review.userId}</span>
                  </div>
