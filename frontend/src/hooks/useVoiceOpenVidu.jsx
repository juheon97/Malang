...
import voiceOpenViduApi from '../api/voiceOpenViduApi';

const useVoiceOpenVidu = (channelId, userName) => {
  ...
  const createAndJoinSession = async (channelId) => {
    ...
    try {
      ...
      const sessionId = await voiceOpenViduApi.createSession(channelId);
      const token = await voiceOpenViduApi.getToken(stringChannelId);
  ...
  const joinExistingSession = async () => {
    ...
    try {
      ...
      const token = await voiceOpenViduApi.getToken(channelId);
...
export default useVoiceOpenVidu;