    ...
    if (requestUserInfo) {
      ...
      try {
        ...
        if (success) {
          ...
          // 5. 준비 안내
          alert(
            '상담 요청을 수락했습니다. 상담자가 입장할 때까지 잠시 기다려주세요.',
          );
        } else {
  ...
  useEffect(() => {
    if (!hasJoined.current && !isLoading) {
      ...
      if (isCounselor) {
        ...
        // 일반 사용자의 경우 승인 여부 확인
        const isApproved =
          sessionStorage.getItem('counselSessionApproved') === 'true';
        const approvedCode = sessionStorage.getItem('approvedCounselorCode');
        const approvalTime = parseInt(
          sessionStorage.getItem('approvalTimestamp') || '0',
        );
